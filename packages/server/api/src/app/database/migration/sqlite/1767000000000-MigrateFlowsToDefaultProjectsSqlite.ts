 ...
 */
export class MigrateFlowsToDefaultProjectsSqlite1767000000000 implements MigrationInterface {
    name = 'MigrateFlowsToDefaultProjectsSqlite1767000000000'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        for (const user of users) {
            ...
            const userProjects = await queryRunner.query(`
                ...
                FROM "project"
                WHERE "ownerId" = ? AND "deleted" IS NULL
            `, [user.id])
        ...
        for (const flow of orphanedFlows) {
            ...
            // First, try to find any project in the same platform
            const platformIdResult = await queryRunner.query(`
                SELECT "platformId" FROM "flow" WHERE id = ?
            `, [flow.id])
            
            const platformId = flow.platformId || platformIdResult[0]?.platformId
            
            if (platformId) {
                const platformProjects = await queryRunner.query(`
                    SELECT p.id, p."ownerId"
                    FROM "project" p
                    WHERE p."platformId" = ? AND p."deleted" IS NULL
